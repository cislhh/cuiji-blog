  ...
  return (
    <div className="space-y-2 pt-6 pb-8 md:space-y-5">
      <nav className="flex items-center justify-between gap-4">
        {!prevPage ? (
          <button
            className="cursor-not-allowed rounded-lg bg-gray-800 px-4 py-2 text-gray-400 transition-all duration-200"
            disabled
          >
            上一页
          </button>
          ...
          <Link
            ...
            rel="prev"
            className="bg-primary-500/10 hover:bg-primary-500/20 text-primary-400 inline-flex cursor-pointer items-center gap-2 rounded-lg px-4 py-2 transition-all duration-200"
          >
            上一页
          </Link>
        )}
            ...
            <div className="sticky top-24 space-y-6">
              ...
              <div className="glass-panel rounded-xl p-6 shadow-lg">
                <div className="mb-4 flex items-center gap-2">
                  ...
                  <h3 className="font-bold text-gray-100">文章分类</h3>
                </div>
                ...
                >
                  {pathname.startsWith('/blog') ? (
                    ...
                    <Link
                      href={`/blog`}
                      className="block cursor-pointer rounded-lg px-3 py-2 text-gray-300 transition-colors duration-200 hover:bg-gray-800"
                    >
                      全部文章
                    </Link>
                  )}
              ...
              {/* 统计信息卡片 */}
              <div className="glass-panel rounded-xl p-6 shadow-lg">
                <h3 className="mb-4 font-bold text-gray-100">博客统计</h3>
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span className="text-gray-400">总文章数</span>
                    <span className="font-semibold text-gray-100">
                      {posts.length}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-400">标签数量</span>
                    <span className="font-semibold text-gray-100">
                      {sortedTags.length}
                    </span>
                  </div>
                  ...
                  >
                    <Link href={`/${path}`} className="block cursor-pointer">
                      <article className="glass-panel hover:border-primary-500/30 h-full rounded-xl border border-transparent p-6 shadow-lg transition-all duration-300 hover:shadow-xl">
                        ...
                        {/* 标题 */}
                        <h2 className="group-hover:text-primary-400 mb-3 text-2xl font-bold text-gray-100 transition-colors duration-200">
                          {title}
                        </h2>
                        ...
                        {/* 阅读更多指示器 */}
                        <div className="text-primary-400 flex items-center gap-2 text-sm font-medium opacity-0 transition-opacity duration-200 group-hover:opacity-100">
                          阅读全文
